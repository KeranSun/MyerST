...
\*Correspondence: Keran Sun (sunkeran@stu.hebmu.edu.cn; ORCID: 0000-0001-6065-8945), Fei Yin (47100214@hebmu.edu.cn; ORCID: 0000-0002-1075-0951)
...
**Comparison with established CCC tools (E14).** On the same receivers and pairs, CellPhoneDB-style population scores detect the CXCL12 association (p = 0.010) but are silent on direction, mechanism and per-receiver variation, and miss PTN-SDC4 entirely (p = 0.83); a NicheNet-style neighbourhood-ligand correlation recovers the CXCL12-location association (r = -0.46) only when the correct (location) endpoint is chosen - the target-semantics choice our framework formalizes - and shows no signal on the mismatched cytotoxicity endpoint (r = +0.01). MyerST's host-based occlusion is thus complementary to database-driven scoring: per-receiver, direction-aware and validated by perturbation of the host itself (Extended Data).
...
**Fig. 4 | CXCL12-mediated T-cell exclusion in Xenium breast cancer.** a, Full-section H&E with the analysis ROI boxed. b, H&E of the 1.5-mm analysis window (H&E images from the public 10x Genomics Xenium dataset (Janesick et al., 2023), reused under CC BY 4.0, cropped to the analysis window). c, Cell types of the tumour-immune interface (12,706 cells). d, T-cell infiltration states vs CXCL12-expressing cells. e, Per-T-cell CXCL12 effect (psi_gene) geography. f, Pathway-level ligand effects on T-cell location (paired t annotated in bars). g, CXCL12 effect decomposed by sender cell type. h, Comparison with established CCC tools on the same pairs (MyerST |t| vs CellPhoneDB-style -log10(p) vs NicheNet-style |r|; PTN-SDC4 is missed by the population-level method). i, PTN->SDC4 effect across cohorts.
