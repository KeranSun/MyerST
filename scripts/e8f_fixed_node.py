"""E8d: multi-seed node benchmark, margin-target game (root fix).
...
def run(regime, kw, sim_seed):
    ...
    # class-NEUTRAL baseline: global mean over all spots. A domain-mean
    # baseline injects class-centroid signal and inverts the credit of
    # informative-but-noisy spots (e8d lesson).
    ...
    ig_exp = IGExplainer(40).explain(adapter, x, target_margin, baseline=baseline_x)
    ig_node = np.abs(ig_exp.meta["node_level"]).sum(1)[players]   # magnitude
    attn = attention_node_scores(gat, x, adj_bin, interface)[players]
    phi = MyersonExplainer(n_samples=256, perm_batch=32, fwd_chunk=16,
                           seed=0).explain(
        adapter, x, target_margin, players=players, edges=data.edges,
        n_spots=data.n_spots, baseline=baseline_x,
        boundary_idx=interface).node_scores
    rng = np.random.default_rng(0)

    with torch.no_grad():
        ref = _target_from_logits(adapter.forward(x), target_margin, interface).item()
    assert ref > 0.3, f"margin target degenerate: ref={ref}"
    rank = {"Myerson": np.abs(phi), "IG-node": ig_node, "attention": attn,
            "random": rng.random(len(players))}
    print(f"    [diag] |phi|: interface {np.abs(phi)[truth].mean():.4f} vs "
          f"1-hop {np.abs(phi)[~truth].mean():.4f}", flush=True)
    p1 = {m: auroc(s, truth) for m, s in rank.items()}

    ev = FaithfulnessEvaluator(adapter, x, baseline_x, target_margin, interface)
    p2 = {}
    for m, s in rank.items():
        cur = ev.node_curve(s, players)
        # raw decay AUC: area under (ref - value) vs masked fraction (no division)
        decay = cur["ref"] - cur["values"]
        trapz = getattr(np, "trapezoid", None) or np.trapz
        p2[m] = float(trapz(decay, cur["frac"]))
...
def main():
    ...
    with open("data/processed/e8f_node_multiseed.pkl", "wb") as f:
        pickle.dump(out, f)
