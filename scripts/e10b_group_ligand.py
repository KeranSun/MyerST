"""E10b: group-ligand marginal — perturb at the model's aggregation granularity.

E10 showed single-edge psi dilutes to the generic edge effect (CXCL12 z=0.25).
The model aggregates over whole neighborhoods, so the right perturbation is
the LIGAND GROUP: for receiver j and pathway (L,R), compare

    psi_group(j) = v({S_L, j}) - v({j})

where S_L = ligand-expressing senders in j's zone. Pathway score = mean over
receivers with R+ and >=1 L+ neighbor; null = random genes with matched
sender-frequency. Same host/data as E10 (cytotoxic T-cell target).
"""
...
def main():
    ...
    print("=" * 72)
    print("E10b: group-ligand marginal on Xenium CCC")
    print("=" * 72)
    ...
    t_act[is_t] = (cyto_score[is_t] > np.median(cyto_score[is_t])).astype(int)
